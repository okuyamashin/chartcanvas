...
/**
 * 007-2.htmlと同じ円グラフをCLIで生成するテスト（大量データ版）
...
const CONFIG_FILE = path.join(__dirname, 'test-cli-configs', 'test-piechart-large.json');
 ...
 */
async function main() {
    console.log('=== 007-2.htmlと同じ円グラフをCLIで生成（大量データ版） ===');
    ...
    try {
        ...
        // SVGファイルを保存
        const svgPath = path.join(TEST_OUTPUT_DIR, 'test-007-1-piechart.svg');
        fs.writeFileSync(svgPath, svgString, 'utf-8');
        console.log(`  SVGを保存: ${svgPath}`);
        
        // PNGに変換
        console.log('PNGに変換中...');
        const pngPath = path.join(TEST_OUTPUT_DIR, 'test-007-1-piechart.png');
